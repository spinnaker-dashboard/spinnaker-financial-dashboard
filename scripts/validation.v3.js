function runBigQueryValidationv3() {
  ...
  // Month-to-Date (I)
  sheet.getRange("I7").setValue(numVal(rawMTD.GPW));
  sheet.getRange("I8").setValue(numVal(rawMTD.GPE));
  sheet.getRange("I9").setValue(numVal(rawMTD.Collected));
  sheet.getRange("I10").setValue(numVal(rawMTD.PolicyCount));
  sheet.getRange("I13").setValue(numVal(rawMTD.PaidLoss));
  sheet.getRange("I14").setValue(numVal(rawMTD.PaidExpense));
  sheet.getRange("I15").setValue(numVal(rawMTD.ClaimCount));

  // BigQuery Previous (I now J), Current (J now K)
  sheet.getRange("J7").setValue(numVal(policiesPrev.GPW));
  sheet.getRange("K7").setValue(numVal(policiesCurr.GPW));
  sheet.getRange("J8").setValue(numVal(policiesPrev.GPE));
  sheet.getRange("K8").setValue(numVal(policiesCurr.GPE));
  sheet.getRange("J9").setValue(numVal(cashPrev.CollectedPremium));
  sheet.getRange("K9").setValue(numVal(cashCurr.CollectedPremium));
  sheet.getRange("J10").setValue(numVal(policiesPrev.policy_count));
  sheet.getRange("K10").setValue(numVal(policiesCurr.policy_count));
  sheet.getRange("J13").setValue(numVal(claimsPrev.loss_paid));
  sheet.getRange("K13").setValue(numVal(claimsCurr.loss_paid));
  sheet.getRange("J14").setValue(numVal(claimsPrev.alae_paid));
  sheet.getRange("K14").setValue(numVal(claimsCurr.alae_paid));
  sheet.getRange("J15").setValue(numVal(claimsPrev.claim_count));
  sheet.getRange("K15").setValue(numVal(claimsCurr.claim_count));
  ...
  sheet.getRange("L7").setValue(growth(numVal(policiesCurr.GPW), numVal(policiesPrev.GPW)));
  sheet.getRange("L8").setValue(growth(numVal(policiesCurr.GPE), numVal(policiesPrev.GPE)));
  sheet.getRange("L9").setValue(growth(numVal(cashCurr.CollectedPremium), numVal(cashPrev.CollectedPremium)));
  sheet.getRange("L10").setValue(growth(numVal(policiesCurr.policy_count), numVal(policiesPrev.policy_count)));
  sheet.getRange("L13").setValue(growth(numVal(claimsCurr.loss_paid), numVal(claimsPrev.loss_paid)));
  sheet.getRange("L14").setValue(growth(numVal(claimsCurr.alae_paid), numVal(claimsPrev.alae_paid)));
  sheet.getRange("L15").setValue(growth(numVal(claimsCurr.claim_count), numVal(claimsPrev.claim_count)));
  ...
  deltaMetrics.forEach(function(metric) {
    ...
    sheet.getRange("M" + metric.row).setValue(delta);
    var status = (delta === 0) ? "Match" : "Mismatch";
    sheet.getRange("N" + metric.row).setValue(status);
  });
